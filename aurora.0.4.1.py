...
def Show_data(power, ffile, currTime_str, daily_power):
    ...
    for element in daily_power:
        new_info = f"""<TD height={SCREEN_HEIGHT}><IMG src="pixelv.jpg" width={PROD_VIEW_WIDTH/(len(daily_power) + 4)} height= {element[1] / TOP_POWER * SCREEN_HEIGHT}></TD>"""
        day_prod_info_list.append(new_info)
        new_info = f"""<TD height={SCREEN_HEIGHT}><IMG src="pixelb.jpg" width={PROD_VIEW_WIDTH / (len(daily_power) + 4)} height= {element[2] / TOP_POWER * SCREEN_HEIGHT}></TD>"""
        day_prod_info_list.append(new_info)
    day_prod_info_list.append("</TD></TR></TABLE>")
...
def actual_prod_data(daily_data):
    first_reading = True
